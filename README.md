...
Each file in the repository contains a program demonstrating a UGOT function.
The function names indicate the type of UGOT used in the program as follows:
- SB: self-balancing vehicle
- WL: wheel-legged robot
- mec: mecanum wheel car

For example, `line_follow.py` contains code for any UGOT robot to follow a line.
Within `line_follow.py`, the function `line_follow_SB()` is used specifically for the self-balancing car.
...
If there is no robot specified in the name, the code is valid for all types of robots.
...
4. Create a new `.ipynb` file and paste the following command in a cell: `pip install ugot opencv-python numpy`. Click the triangle next to the cell to execute. You only need to do this once per computer.![alt text](install.png)
